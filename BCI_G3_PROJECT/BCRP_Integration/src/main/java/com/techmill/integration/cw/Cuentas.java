...
public class Cuentas {
	static Log log = new Log("CWCuentas");

	public RootCuentas processRequest(String sid, String institucionID) throws IOException {
		String T24Input = "/" + institucionID + "/cuentas?sid=" + sid;
		InputStream input = Cuentas.class.getClassLoader().getResourceAsStream("config/bcrpConfig.properties"); // Read
		...
		prop.load(input);
		String Base_URL = prop.getProperty("Cuentas_URL");
		...
		try {
			...
			RootCuentas root = gson.fromJson(element, RootCuentas.class);
