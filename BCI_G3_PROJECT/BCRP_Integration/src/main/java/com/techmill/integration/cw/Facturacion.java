...
public class Facturacion {
	static Log log = new Log("CWFacturacion");

	public RootFacturacion processRequest(String sid, String periodo) throws IOException {
		String T24Input = "sid=" + sid + "&periodo=" + periodo;
		InputStream input = Facturacion.class.getClassLoader().getResourceAsStream("config/bcrpConfig.properties"); // Read
		...
		prop.load(input);
		String Base_URL = prop.getProperty("Facturacion_URL");
		...
		try {
			...
			RootFacturacion root = gson.fromJson(element, RootFacturacion.class);
