...
public class Movimientos {
	static Log log = new Log("CWMovimiento");

	public RootMovimientos processRequest(String fecha, String sid, String cuenta) throws IOException {
		InputStream input = Movimientos.class.getClassLoader().getResourceAsStream("config/bcrpConfig.properties"); // Read
		...
		prop.load(input);
		String Base_URL = prop.getProperty("Movimientos_URL");
		String AuthKey = prop.getProperty("AuthKey");
		String dataIn = "";
		String dataOut = "";
		String final_URL = Base_URL + "/" + cuenta + "/movimientos?fecha=" + fecha + "&sid=" + sid;
		...
		try {
			...
			RootMovimientos root = gson.fromJson(element, RootMovimientos.class);
