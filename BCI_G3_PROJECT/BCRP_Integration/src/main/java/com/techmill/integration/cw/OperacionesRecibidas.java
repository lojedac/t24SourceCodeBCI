...
public class OperacionesRecibidas {
	static Log log = new Log("CWOperacionesRecibidas");

	public RootOperacionesRecibidas processRequest(String sid, String fechaLiquidacion) throws IOException {
		String data = "&fechaLiquidacion=" + fechaLiquidacion + "&sid=" + sid;
		InputStream input = OperacionesRecibidas.class.getClassLoader()
				.getResourceAsStream("config/bcrpConfig.properties");
		...
		prop.load(input);
		String Base_URL = prop.getProperty("OperacionesRecibidas_URL");
		...
		try {
			...
			RootOperacionesRecibidas root = gson.fromJson(element, RootOperacionesRecibidas.class);
