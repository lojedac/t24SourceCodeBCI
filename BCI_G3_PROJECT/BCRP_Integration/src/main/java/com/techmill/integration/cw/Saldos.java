...
public class Saldos {
	static Log log = new Log("CWSaldos");

	public RootSaldos processRequest(String sid, String numCuenta, String fecha) throws IOException {
		String data = "/" + numCuenta + "/saldos?fecha=" + fecha + "&sid=" + sid;
		InputStream input = Saldos.class.getClassLoader().getResourceAsStream("config/bcrpConfig.properties"); // Read
		...
		prop.load(input);
		String Base_URL = prop.getProperty("Saldos_URL");
		...
		try {
			...
			RootSaldos root = gson.fromJson(element, RootSaldos.class);
