 ...
 * @author anagha.s
 *         ----------------------------------------------------------------------------------------------------------------
 *         Description : This routine is used to generate the outward Returned
 ...
 *
 *         Attached To : BATCH>BciCCEOutClrRtn
 *
 ...
 *         -----------------------------------------------------------------------------------------------------------------
 */
public class BciCCEOutClrRtnTRT extends ServiceLifecycle {
        ...
        try {
            ...
            if (today.equals(date)) {
                ...
                List<String> recList = da.selectRecords("", "EB.BCI.CCE.MAPPING.FIELD.VALUES", "",
                        "WITH STATUS EQ TO_BE_RETURNED_IN_OFS OR STATUS EQ TO_BE_CONFIRMED_IN_PARTIAL AND DATE EQ "
                                + today + " AND TXN.TYPE EQ " + appcode);
                ArrayList<String> finalCCEStr = new ArrayList<String>();
                ...
                ArrayList<String> lotendHeader220Pen = new ArrayList<String>();
                ArrayList<String> lotendHeader221Pen = new ArrayList<String>();
                ...
                for (String recid : recList) {
                    ...
                    String lastdat = bciFldRec.getDate().getValue();
                    if (lastdat.equals(today)) {

                        System.out.println("REC DATE: " + lastdat);
                        System.out.println("TODAY DATE: " + today);
                        String oeid = bciFldRec.getOeId().getValue();
                        String stat = bciFldRec.getStatus().getValue();
                        PpOrderEntryRecord payRec = new PpOrderEntryRecord(this);
                        System.out.println("OE id: " + oeid);
                        if (stat.equals("TO_BE_CONFIRMED_IN_PARTIAL")) {
                            payRec = new PpOrderEntryRecord(da.getRecord("PP.ORDER.ENTRY", oeid));
                        }
                        if (stat.equals("TO_BE_RETURNED_IN_OFS")) {
                            payRec = new PpOrderEntryRecord(da.getRecord("", "PP.ORDER.ENTRY", "$NAU", oeid));
                        }
                        System.out.println("record: " + payRec);
                        ...
                        for (FieldNameClass fieldid : fldList) {
                            ...
                            if (fieldName.equals("IMMEDIATE.DESTINATION.CODE.ECTRMD")) {
                        ...
                        String coin = payRec.getTransactioncurrency().getValue();
                        if (coin.equals("PEN")) {
                            currency = currPen;
                        }
                        if (coin.equals("USD")) {
                        ...
                        System.out.println("archiveNumber 2: " + archiveNumber);
                        try {
                         ...
                         * immOrigin = immOrigin.substring(0, 7); } catch
                         * (Exception e) {
                         * 
                         * }
                         **/
                        ...
                        // payRec.getDebitvaluedate().getValue();
                        ...
                        for (FieldNameClass fieldid : fldList1) {
                            ...
                            if (fieldName.equals("LOT.TYPE.RETURN")) {
                        ...
                        try {
                            ...
                            // archiveNumberLH =
                            // headCntrRec.getName().get(0).getValue().getValue();

                            archiveNumberLH = headCntrRec.getName(2).getValue().getValue();
                        } catch (Exception e) {
                        ...
                        try {
                            ...
                            // datePresentation =
                            // payRec.getDebitvaluedate().getValue();
                            dateOfPresentation = today;
                            // settlementDate =
                            // payRec.getProcessingdate().getValue();
                        ...
                        try {
                             ...
                             * CompanyRecord companyRec = new
                             * CompanyRecord(da.getRecord("COMPANY", company));
                             * String entidadCode =
                             * companyRec.getLocalRefField("L.ENTITY.CODE").
                             * getValue(); String officeCode =
                             * companyRec.getLocalRefField("L.OFFICE.CODE").
                             * getValue(); originEntity =
                             * StringUtils.leftPad(entidadCode, 4, "0")
                        ...
                        String lotHeaderStr = typRegLH.concat(StringUtils.leftPad(copyFileHeader, 2, "0"))
                                .concat(lotType).concat(StringUtils.rightPad(issueCompany, 46, ""))
                                ...
                                .concat(StringUtils.leftPad(lotNumber, 7, "0"));
                        int L = 100;
                        ...
                        String funcError = "";
                        try {
                            funcError = payRec.getFunctionalerror().get(0).getValue();
                        ...
                        try {
                            if (funcError.equals("")) {
                               funcError = payRec.getOverride().get(0).getValue();
                            }
                        }
                        catch (Exception e) {
                            System.out.println(e.getMessage());
                        ...
                        try {
                            if (funcError.equals("")) {
                               funcError = payRec.getReturndescription().getValue();
                            }
                        }
                        catch (Exception e) {
                            System.out.println(e.getMessage());
                        ...
                        try {
                            System.out.println(funcError);
                            final String[] errorSplit1 = funcError.split("/");
                            final String[] errorSplit2 = errorSplit1[0].split("\\|");
                            codeError = errorSplit2[0].trim();
                            System.out.println("CodeError: " + codeError);
                        } catch (Exception e) {

                        }
                        try {
                            final EbBciCceGroundReturnRecord groudReturnRec = new EbBciCceGroundReturnRecord(da.getRecord("EB.BCI.CCE.GROUND.RETURN", "SYSTEM"));
                            final List<CodeClass> codeClassList = (List<CodeClass>)groudReturnRec.getCode();
                            for (final CodeClass codeClass : codeClassList) {
                                final String codeErrorClass = codeClass.getDescription().getValue().trim();
                                if (codeError.equals(codeErrorClass)) {
                                    reasonRet = codeClass.getCode().getValue();
                                    break;
                                }
                            }
                        }
                        ...
                        System.out.println(entOrgTrans);
                        // uniSeq = ordTrnsRecCnt;
                        try {
                            // uniSeq = ordTrnsRecCnt.substring(0, 7);
                            uniSeq = recid;
                        } catch (Exception e) {
                        ...
                        // String ordTrns =
                        // payRec.getLocalRefField("L.REGIST.CONTR").getValue().substring(0,
                        // 8);
                        ...
                        additionalReturnRegistration = typeRegAdd.concat(addReg)
                                ...
                                .concat(StringUtils.leftPad(ordTrns, 8, "0"))
                                .concat(StringUtils.rightPad(addInfo, 44, ""))
                                .concat(StringUtils.leftPad(uniSeq, 7, "0")).concat(StringUtils.rightPad(free, 105, ""))
                                .concat(StringUtils.leftPad(indTransRecCnt, 15, "0"));

                        System.out.println("entered individualadditional" + additionalReturnRegistration);
                        ...
                        for (FieldNameClass fieldid : fldList) {
                            String fieldName = fieldid.getFieldName().getValue();
                            if (fieldName.equals("TRANSACTION.CODE>RETURN")) {
                        ...
                        /*try {
                            ...
                            if (transferType.equals("220")) {
                                if (sameOwner.equals("YES")) {
                                    ...
                                    String transferRef = docTypeValue.concat(docNumber);
                                    transferRef = StringUtils.rightPad(transferRef, 17, "0");
                                    // StringBuilder strbline1 = new StringBuilder(individualRegistration);
                                    // StringBuilder linemod1 = strbline1.replace(152, 169, transferRef);
                                    // individualRegistration = linemod1.toString();
                                }else {
                                    String univoco = individualRegistration.substring(177, 184);
                                    long univocoLong = 0;
                                    try {
                                        univocoLong = Long.parseLong(univoco);
                                        univoco = StringUtils.rightPad(String.valueOf(univocoLong), 24, "0");
                                    } catch (Exception e) {
                                    }
                                    StringBuilder strbline1 = new StringBuilder(individualRegistration);
                                    StringBuilder linemod1 = strbline1.replace(152, 176, univoco);
                                    individualRegistration = linemod1.toString();
                        ...
                        StringBuilder linemod = strbline.replace(185, 193, originEntityLC1);
                        ...
                        if (coin.equals("PEN")) {
                            ...
                            if (transferType.equals("220")) {
                                ...
                                System.out.println("entered individualadditional" + individualAdd220Pen);
                            }
                            if (transferType.equals("221")) {
                                ...
                                System.out.println("entered individualadditional" + individualAdd221Pen);
                            }
                            if (transferType.equals("222")) {
                        ...
                        try {
                            ...
                            if (!commList.isEmpty()) {
                                 ...
                                 * .concat(sumOfCommisions.substring(commAmt +
                                 * 1));
                                 */
                            }
                        } catch (Exception e) {
                        ...
                        if (coin.equals("PEN")) {
                            ...
                            totalOperationsFCPen = ((individualAdd220Pen.size()) / 3)
                                    + ((individualAdd221Pen.size()) / 3) + ((individualAdd222Pen.size()) / 3)
                                    + ((individualAdd223Pen.size()) / 3) + ((individualAdd224Pen.size()) / 3);
                        ...
                        if (coin.equals("USD")) {
                            ...
                            totalOperationsFCUsd = ((individualAdd220usd.size()) / 3)
                                    + ((individualAdd221usd.size()) / 3) + (individualAdd222usd.size() / 3)
                                    + (individualAdd223usd.size() / 3) + (individualAdd224usd.size() / 3);
                        }

                        // String sumOfCommisions = "";
                        String totRecs = Integer.toString(totRec); // need
                                                                   // mapping
                        // String controlTotals =
                        ...
                        /*
                         * sumOfAmountslc =
                         * Double.parseDouble(payRec.getTransactionamount().
                         * getValue()) + sumOfAmountslc; BigDecimal Decimal =
                         * new BigDecimal(sumOfAmountslc);
                         ...
                         * Double.parseDouble(commList.get(1).
                         * getDebitchargeamount().getValue()) + sumOfCommlc;
                         * BigDecimal comDec = new BigDecimal(sumOfCommlc);
                         * sumOfCommisions = String.format("%.02f", comDec); int
                         * commAmt = sumOfCommisions.indexOf(".");
                         * sumOfCommisions = sumOfCommisions.substring(0,
                         * commAmt) .concat(sumOfCommisions.substring(commAmt +
                         * 1)); } } catch (Exception e) {
                         * 
                         * }
                         * 
                         ...
                         * CompanyRecord(da.getRecord("COMPANY", company));
                         * String entidadCode =
                         * companyRec.getLocalRefField("L.ENTITY.CODE").getValue
                         * (); String officeCode =
                         * companyRec.getLocalRefField("L.OFFICE.CODE").getValue
                         * (); String originEntityLC =
                         * StringUtils.leftPad(entidadCode, 4, "0")
                         * .concat(StringUtils.leftPad(officeCode, 4, "0"));
                         */
                        ...
                        try {
                            // originEntityLC = "0" +
                            // originEntityLC.substring(0, 3) + "0"
                            // + originEntityLC.substring(3, 6);
                        ...
                        // sumOfAmounts= Amountsum.toString();
                        // sumOfCommisions= Commsum.toString();
                        // BigDecimal comiamtin2decimal = new
                        // BigDecimal(Commsum);
                        ...
                        String endOfcontrol = typeRegLC.concat(StringUtils.leftPad(totRecs, 10, "0"))
                                ...
                                .concat(StringUtils.leftPad(lotNumber, 7, "0"))
                                .concat(StringUtils.rightPad(free, 91, ""));

                        /*
                         * int Lec = 100; Lec = endOfcontrol.length(); Lec = Lec
                         * + 23; String endofctrl = String.format("%" + -Lec +
                         * "s", endOfcontrol); String endofctrlstr =
                         * endofctrl.concat(originEntityLC).concat(StringUtils.
                         * leftPad( lotNumberLC, 7, "0")); int len = 100; len =
                         * endOfcontrol.length(); len = len + 91; String
                         * finalStr = String.format("%" + -len + "s",
                         * endofctrlstr);
                         */
                        ...
                        mapRecwrite.setStatus("RETURNED_IN");
                ...
                if (!fileHeaderUSD.equals("")) {
                    ...
                    String totalLots = "";
                    String totalRecs = "";
                    String controlTots = "";// need mapping
                    String totOperation = "";// need mapping
                    try {
                        controlTots = String.valueOf(controlTotalFCusd);
                        totalLots = Integer.toString(totalLot);
                        ...
                        totalRecs = Integer.toString(totCnt);

                        int sumlen = sumAmountsusd.indexOf(".");
                        sumAmountsusd = sumAmountsusd.substring(0, sumlen).concat(sumAmountsusd.substring(sumlen + 1));
                        int comlen = commisionAmountusd.indexOf(".");
                        commisionAmountusd = commisionAmountusd.substring(0, comlen)
                                .concat(commisionAmountusd.substring(comlen + 1));
                    } catch (Exception e) {
                ...
                if (!fileHeaderPEN.equals("")) {
                    ...
                    try {
                        ...
                        int sumlen = sumAmountspen.indexOf(".");
                        sumAmountspen = sumAmountspen.substring(0, sumlen).concat(sumAmountspen.substring(sumlen + 1));
                        int comlen = commisionAmountpen.indexOf(".");
                        commisionAmountpen = commisionAmountpen.substring(0, comlen)
                                .concat(commisionAmountpen.substring(comlen + 1));
                    } catch (Exception e) {
                ...
                if (individualCntUSD > 0 || individualCntPEN > 0) {
                    ...
                    for (int indice = 0; indice < individualAdd220Pen.size(); indice++) {
                        ...
                        if (individualAdd220Pen.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd220Pen.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd220Pen.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd220Pen.remove(indice + 2);
                            individualAdd220Pen.add(indice + 2, linemod3.toString());
                        }
                    ...
                    for (int indice = 0; indice < individualAdd221Pen.size(); indice++) {
                        ...
                        if (individualAdd221Pen.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd221Pen.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd221Pen.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd221Pen.remove(indice + 2);
                            individualAdd221Pen.add(indice + 2, linemod3.toString());
                        }
                    ...
                    for (int indice = 0; indice < individualAdd222Pen.size(); indice++) {
                        ...
                        if (individualAdd222Pen.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd222Pen.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd222Pen.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd222Pen.remove(indice + 2);
                            individualAdd222Pen.add(indice + 2, linemod3.toString());
                        }
                    ...
                    for (int indice = 0; indice < individualAdd223Pen.size(); indice++) {
                        ...
                        if (individualAdd223Pen.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd223Pen.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd223Pen.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd223Pen.remove(indice + 2);
                            individualAdd223Pen.add(indice + 2, linemod3.toString());
                        }
                    ...
                    for (int indice = 0; indice < individualAdd224Pen.size(); indice++) {
                        ...
                        if (individualAdd224Pen.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd224Pen.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd224Pen.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd224Pen.remove(indice + 2);
                            individualAdd224Pen.add(indice + 2, linemod3.toString());
                        }
                    ...
                    for (int indice = 0; indice < individualAdd220usd.size(); indice++) {
                        ...
                        if (individualAdd220usd.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd220usd.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd220usd.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd220usd.remove(indice + 2);
                            individualAdd220usd.add(indice + 2, linemod3.toString());
                        }
                    ...
                    for (int indice = 0; indice < individualAdd221usd.size(); indice++) {
                        ...
                        if (individualAdd221usd.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd221usd.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd221usd.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd221usd.remove(indice + 2);
                            individualAdd221usd.add(indice + 2, linemod3.toString());
                        }
                    ...
                    for (int indice = 0; indice < individualAdd222usd.size(); indice++) {
                        ...
                        if (individualAdd222usd.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd222usd.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd222usd.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd222usd.remove(indice + 2);
                            individualAdd222usd.add(indice + 2, linemod3.toString());
                        }
                    ...
                    for (int indice = 0; indice < individualAdd223usd.size(); indice++) {
                        ...
                        if (individualAdd223usd.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd223usd.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd223usd.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd223usd.remove(indice + 2);
                            individualAdd223usd.add(indice + 2, linemod3.toString());
                        }
                    ...
                    for (int indice = 0; indice < individualAdd224usd.size(); indice++) {
                        ...
                        if (individualAdd224usd.get(indice).substring(0, 1).equals("6")) {
                            ...
                            String secUnivoca = individualAdd224usd.get(indice).substring(177, 184);
                            StringBuilder strbline3 = new StringBuilder(individualAdd224usd.get(indice + 2));
                            StringBuilder linemod3 = strbline3.replace(73, 80, secUnivoca);
                            individualAdd224usd.remove(indice + 2);
                            individualAdd224usd.add(indice + 2, linemod3.toString());
                        }
                ...
                // System.out.println("entered FILE endofcontrol " +fileendPen);
                ...
                // System.out.println("entered FINAL: " + finalCCEStr);
                if ((fileHeaderUSDcnt == 0) || (fileHeaderPENcnt == 0)) {
                    ...
                    // Para la secuencia del fileHeader
                    try {
                        if (fileHeaderUSD.size() > 0 && fileHeaderPEN.size() == 0) {
                            String strcountUSD = fileHeaderUSD.get(0).substring(30, 32);
                    ...
                    String fileControlStrPen = typeRegEC.concat(StringUtils.leftPad(totalLots, 6, "0"))
                            ...
                            .concat(StringUtils.leftPad(commisionAmountpen, 15, "0"));
                    int Lfc = 100;
                    Lfc = fileControlStrPen.length();
                    Lfc = Lfc + 123;
                    fileendcontrol = String.format("%" + -Lfc + "s", fileControlStrPen);
                 ...
                 * (fieldName.equals("OUTWARD.RETURN")) { returnedfile =
                ...
                // outPath =
                // "/project/BackupData/BACKUP/BCIISB/BNK/UD/CCE/OUTWARD/ECTRMD.TXT";
                // System.out.println("wRITING THE FILE: " + outPath);
